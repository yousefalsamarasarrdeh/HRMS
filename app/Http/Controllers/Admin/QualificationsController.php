...
class QualificationsController extends Controller
{
    ...
    public function store(QualificationsRequest $request)
    {
        try {
            ...
            if (!empty($checkExsists)) {
                return redirect()->back()->with(['status' => 'عفوا هذا الاسم مسجل من قبل !!'])->withInput();
            }
            DB::beginTransaction();
    ...
    public function edit($id)
    {
        ...
        if (empty($data)) {
            return redirect()->route('Qualifications.index')->with(['status' => 'عفوا غير قادر للوصول الي البيانات المطلوبة !']);
        }
    ...
    public function update($id, QualificationsRequest $request)
    {
        try {
            ...
            if (!empty($checkExsists)) {
                return redirect()->back()->with(['status' => 'عفوا هذه الاسم مسجل من قبل '])->withInput();
            }
            DB::beginTransaction();
            ...
            DB::rollBack();
            return redirect()->back()->with(['status' => 'عفوا حدث خطأ ' . $ex->getMessage()])->withInput();
        }
    }
    public function destroy($id)
    {
        try {
            ...
            DB::rollBack();
            return redirect()->back()->with(['status' => 'عفوا حدث خطأ ' . $ex->getMessage()]);
        }
    }
}
